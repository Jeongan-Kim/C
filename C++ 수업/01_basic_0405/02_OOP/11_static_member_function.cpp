...
public:
	...
	static void Temp2() { value2; } //변수도 static 변수만 쓸 수 있다.

	int test;
};
int main()
{
	//Test::Temp1(); 멤버 함수를 호출할떄, 이런 방식은 불가능(객체가 실체화되야 함)
	Test t1;
	t1.Temp1(); //이렇게 했어야 했는데,

	Test::Temp2(); //static함수는 이렇게 호출 가능. 소속은 표현하되, 일반 함수 취급을 한다고 생각하면 됨.
	//그래서 함수 포인터도 가능
